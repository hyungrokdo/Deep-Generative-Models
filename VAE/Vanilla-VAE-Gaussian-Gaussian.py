...
def decoder(z_in, use_batchnorm=True, use_bias=True):
    ...
    with tf.variable_scope('vae/decoder', reuse=reuse):
        ...
        mean   = tf.layers.conv2d_transpose(inputs=net, filters=1, kernel_size=(1, 1), strides=(1, 1), use_bias=use_bias, padding='same',
                                            kernel_initializer=xavier_init_conv, name='layer4/x_mean', reuse=reuse)
        logvar = tf.layers.conv2d_transpose(inputs=net, filters=1, kernel_size=(1, 1), strides=(1, 1), use_bias=use_bias, padding='same',
...
log_recon_prob = -0.5*(tf.reduce_sum(tf.squared_difference(x_in, x_mean)*tf.exp(-x_logvar), axis=[1, 2, 3]) + tf.reduce_sum(x_logvar, axis=[1, 2, 3]) + 784*np.log(2*np.pi))
...
for epoch in pbar:
    ...
    loss_stack, kld_stack, log_recon_prob_stack, recon_prob_stack = [], [], [], []
    ...
    pbar.set_description('Loss: {:.4f} | KLD: {:.4f} | Log-Recon-Prob: {:.4f}'.format(np.mean(loss_stack),
                                                                                      np.mean(kld_stack), np.mean(log_recon_prob_stack)))
