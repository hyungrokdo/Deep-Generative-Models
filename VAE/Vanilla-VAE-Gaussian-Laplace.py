# -*- coding: utf-8 -*-
"""
Created on Thu Nov 22 15:55:38 2018

@author: Hyungrok Do
         hyungrok.do11@gmail.com
         https://github.com/hyungrokdo
         
         A tensorflow impelementation of Gaussian encoder - Laplace decoder variational autoencoder (Kingma and Welling, 2013)
"""
...
mnist = input_data.read_data_sets('d:/fashion_mnist_data/', one_hot=False, reshape=False)
z_dim = 25
...
def decoder(z_in, use_batchnorm=True, use_bias=True):
    ...
    with tf.variable_scope('vae/decoder', reuse=reuse):
        ...
        x_logscale = tf.layers.conv2d_transpose(inputs=net, filters=1, kernel_size=(5, 5), strides=(1, 1), use_bias=use_bias, padding='same',
                                               kernel_initializer=xavier_init_conv, name='layer4/x_logscale', reuse=reuse)
        x_logscale = tf.clip_by_value(x_logscale, -2, 10, name='layer4/x_logscale_clipped')
    return x_mean, x_logscale
...
x_mean, x_logscale = decoder(z_sample)

kl_divergence  = -0.5*tf.reduce_sum(1+z_logvar-tf.square(z_mean)-tf.exp(z_logvar), axis=1)
log_recon_likelihood = -0.5*(tf.reduce_sum(tf.abs(x_in-x_mean)*tf.exp(-x_logscale), axis=[1, 2, 3]) + tf.reduce_sum(x_logscale, axis=[1, 2, 3])) + 784*np.log(2)
...
for epoch in pbar:
    ...
    pbar.set_description('Loss: {:.4f} | KLD: {:.4f} | Log-Recon-Likelihood: {:.4f}'.format(np.mean(loss_stack), np.mean(kld_stack), np.mean(log_recon_likelihood_stack)))
