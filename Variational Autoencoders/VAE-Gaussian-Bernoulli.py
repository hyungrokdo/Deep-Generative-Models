...
def decoder(z_in, use_batchnorm=True, use_bias=True):
    ...
    with tf.variable_scope('vae/decoder', reuse=reuse):
        ...
        logit   = tf.layers.conv2d_transpose(inputs=net, filters=1, kernel_size=(5, 5), strides=(1, 1), use_bias=use_bias, padding='same',
...
log_recon_likelihood = tf.reduce_sum(x_prob*x_discrete + (1-x_prob)*(1-x_discrete), axis=[1, 2, 3])
